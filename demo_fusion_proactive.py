...
async def demo_integrated_system():
    ...
    print(
        "  [ANALYSIS]: Critical situation detected - presentation failure + high stress"
    )
    ...
    try:
        for i in range(5):
            ...
            if i == 0:
                ...
                print(
                    f"[{datetime.now().strftime('%H:%M:%S')}] Potential fatigue detected"
                )
                print("  → JARVIS: Suggesting micro-break")
            elif i == 3:
                print(
                    f"[{datetime.now().strftime('%H:%M:%S')}] Meeting preparation needed"
                )
