...
async def demo_basic_optimization():
    ...
    print(
        f"  Entanglement strength: {result['collective_metrics']['final_entanglement']:.4f}"
    )
    print(
        f"  Knowledge transfers: {result['collective_metrics']['knowledge_transfers']}"
    )
    print(
        f"  Active entanglements: {result['collective_metrics']['active_entanglements']}"
    )
...
async def demo_quantum_features():
    ...
    standard_optimizer = QuantumSwarmOptimizer(
        n_agents=20, dimension=10, topology="fully_connected", max_iterations=300
    )
    ...
    quantum_optimizer = QuantumSwarmOptimizer(
        n_agents=20, dimension=10, topology="small_world", max_iterations=300
    )
    ...
    print(
        f"  Tunneling events: {quantum_result['quantum_metrics']['tunneling_events']}"
    )

    improvement = (
        (quantum_result["best_fitness"] - standard_result["best_fitness"])
        / abs(standard_result["best_fitness"])
        * 100
    )
...
async def demo_ensemble_optimization():
    ...
    ensemble = QuantumSwarmEnsemble(
        n_swarms=5,
        swarm_config={"n_agents": 20, "dimension": n_assets, "max_iterations": 200},
    )
    ...
    result = await ensemble.optimize_ensemble(
        portfolio_objective, bounds, consensus_interval=50
    )
    ...
    print(
        f"  Consensus confidence: {result['ensemble_stats']['consensus_confidence']:.2%}"
    )
...
async def demo_real_time_adaptation():
    ...
    for t in range(5):
        ...
        print(
            f"  Quantum coherence: {result['collective_metrics']['final_coherence']:.3f}"
        )
