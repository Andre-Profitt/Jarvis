...
class MicroAgentSwarm:
    ...
    def create_agent(
        ...
        if model is None:
            model = self.model_nursery.create_specialized_model(
        ...
        for capability in missing_capabilities:
            ...
            if await self._needs_new_agent(capability):
                ...
                new_agent = self.create_agent(
                    agent_type, capabilities=[capability], parent_agent=parent.id
                )
...
class ModelNursery:
    ...
    async def train_specialized_model(self, capability: str, data_path: str) -> Any:
...
class AgentToAgentProtocol:
    ...
    async def negotiate_task(
        self, agents: List[MicroAgent], task: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        best_proposal = max(
            proposals,
            key=lambda p: sum(1 for v in votes.values() if v == p["agent"].id),
        )
