...
class JARVISLauncher:
    ...
    async def _check_dependencies(self, service_name: str) -> bool:
        ...
        for dep in service.dependencies:
            if dep not in self.services:
                self.logger.error(f"Service {service_name} has unknown dependency: {dep}")
                return False
            
            dep_service = self.services[dep]
            if not dep_service.enabled:
                self.logger.warning(f"Service {service_name} dependency {dep} is disabled")
                return False
            
            if dep_service.status not in (ServiceStatus.RUNNING, ServiceStatus.DEGRADED):
                self.logger.warning(f"Service {service_name} dependency {dep} is not running")
                return False
    ...
    async def monitor_health(self):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                # Log health status
                healthy_count = sum(1 for s in health_report['services'].values() 
                                  if s['status'] == 'running')
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                if command == 'exit':
                    ...
                    if 'multi_ai' in self.services and self.services['multi_ai'].instance:
                        try:
                            response = await self.services['multi_ai'].instance.process(user_input)
                            print(f"\n{response}\n")
                        except Exception as e:
    ...
    async def _cmd_status(self):
        ...
        for name, service in self.services.items():
            ...
            print(f"{status_icon} {name}: {service.status.value} [{enabled_str}]{required_str}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='+',
        help='Only start specific services'
    )
    parser.add_argument(
        '--log-level',
        type=str,
