...
class JARVISDebugServer:
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.debug(
                        f"Message #{self.message_count} from {client_id}: {message}"
                    )
            ...
            logger.info(
                f"Cleaned up {client_id}. Active connections: {len(self.active_connections)}"
            )
    ...
    async def start_server(self):
        ...
        server = await websockets.serve(
            self.handle_connection, "localhost", 8765, ping_interval=20, ping_timeout=10
        )
