...
from autonomous_project_engine import (
    AutonomousProjectEngine, BaseAgent, AgentCapability, 
    AgentRole, Task, ProjectContext
)
...
class DatabaseArchitectAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str):
        capabilities = [
            AgentCapability("schema_design", 0.9, ["postgresql", "mongodb"], ["relational", "nosql"]),
            AgentCapability("query_optimization", 0.85, ["explain", "indexing"], ["performance"]),
            AgentCapability("data_modeling", 0.88, ["normalization", "denormalization"], ["scalability"])
        ]
    ...
    async def execute(self, task: Task, context: ProjectContext) -> Dict[str, Any]:
        ...
        schema_design = {
            "tables": {
                ...
                "products": {
                    "columns": ["id", "name", "description", "price", "inventory", "category_id"],
    ...
    async def learn_from_experience(self, experience: Dict[str, Any]) -> None:
        ...
        if "query_performance" in experience:
            ...
            if avg_query_time > 100:  # ms
                ...
                self.capabilities[1].skill_level = min(0.95, self.capabilities[1].skill_level * 1.02)
...
class SecurityAuditAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str):
        capabilities = [
            AgentCapability("vulnerability_scanning", 0.92, ["owasp", "burp"], ["web_security"]),
            AgentCapability("authentication_design", 0.88, ["oauth", "jwt"], ["identity"]),
            AgentCapability("encryption_implementation", 0.9, ["aes", "rsa"], ["data_protection"]),
            AgentCapability("compliance_verification", 0.85, ["pci", "gdpr"], ["regulations"])
        ]
...
class PerformanceOptimizationAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str):
        capabilities = [
            AgentCapability("load_testing", 0.87, ["jmeter", "locust"], ["stress_testing"]),
            AgentCapability("caching_strategy", 0.9, ["redis", "memcached"], ["optimization"]),
            AgentCapability("cdn_configuration", 0.85, ["cloudflare", "fastly"], ["distribution"]),
            AgentCapability("code_profiling", 0.88, ["profilers", "apm"], ["bottlenecks"])
        ]
        super().__init__(agent_id, AgentRole.EXECUTOR, capabilities)
    ...
    async def learn_from_experience(self, experience: Dict[str, Any]) -> None:
        ...
        if "actual_improvements" in experience:
            ...
            for metric, actual in experience["actual_improvements"].items():
                ...
                self.performance_metrics[f"prediction_accuracy_{metric}"].append(accuracy)
...
async def run_ecommerce_project():
    ...
    if insights.get('agent_performances'):
        for agent, perf in insights['agent_performances'].items():
            print(f"  - {agent}: {perf.get('trend', 'stable')} "
