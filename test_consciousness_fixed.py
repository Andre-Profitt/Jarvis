...
async def test_consciousness():
    ...
    print(f"   Language processing: {language_concept.content.get('inner_speech', 'Processing...')}")
    
    # Test motor module
    motor = simulator.modules['motor']
    motor_concept = await motor.process({'action': 'reach'})
    print(f"   Motor planning: {len(motor_concept.content['motor_plan']['steps'])} steps")
    ...
    if hasattr(simulator, 'experience_history') and simulator.experience_history:
        ...
        print(f"   Workspace concepts: {len(latest_experience.global_workspace_content)}")
    ...
    print("The consciousness simulator now uses 'simulate_consciousness_loop()' correctly.")
