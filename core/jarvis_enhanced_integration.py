...
class JARVISEnhancedCore:
    ...
    async def process_request(
        ...
        user_context = UserContext(
            ...
            last_break_minutes_ago=(
                context.get("last_break_minutes_ago", 0) if context else 0
            ),
        ...
        intervention = await self.emotions.check_intervention_needed(
    ...
    def _classify_request(self, text: str) -> str:
        ...
        if any(
            word in text_lower
            for word in ["code", "function", "program", "implement", "create function"]
        ):
            return "code_synthesis"
        elif any(
            word in text_lower
            for word in ["feeling", "stressed", "tired", "happy", "sad", "emotion"]
        ):
            return "emotional_support"
        elif any(
            word in text_lower
            for word in ["status", "health", "performance", "metrics"]
        ):
            return "system_status"
        elif any(
            word in text_lower for word in ["family", "brother", "birthday", "promise"]
        ):
    ...
    async def _handle_emotional_support(self, emotional_state) -> Dict[str, Any]:
        ...
        family_response = self.emotions.get_family_aware_response(
...
async def demo_enhanced_jarvis():
    ...
    try:
        ...
        response1 = await jarvis_process(
            ...
            context={"work_duration_hours": 4, "last_break_minutes_ago": 150},
        )
