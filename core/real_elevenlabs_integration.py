...
class RealElevenLabsIntegration:
    ...
    async def speak(
        ...
        try:
            ...
            voice_config = self.voices_config.get(
                voice_key, self.voices_config["jarvis_main"]
            )
            ...
            # Generate audio
            if streaming:
                audio_data = await self._generate_streaming(
                    text, voice_config, settings
                )
            else:
    ...
    async def _generate_standard(
        self, text: str, voice_config: Dict[str, Any], settings: Dict[str, Any]
    ) -> bytes:
    ...
    def _clean_cache(self):
        ...
        if total_size > 100 * 1024 * 1024:  # 100MB
            # Remove oldest files
            files = sorted(
                self.cache_dir.glob("*.mp3"), key=lambda f: f.stat().st_mtime
            )
    ...
    async def create_custom_voice(
        self, name: str, audio_files: List[str]
    ) -> Dict[str, str]:
        ...
        try:
            ...
            voice = self.client.voices.add(
                name=name, files=files, description=f"Custom voice for JARVIS - {name}"
            )
