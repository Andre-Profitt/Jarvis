...
class ProactiveActionV2:
    ...
    multi_modal_requirements: List[str] = field(default_factory=list)  # Required modalities
...
class EnhancedContextualMemory:
    ...
    def __init__(self, memory_size: int = 10000):
        ...
        try:
            self.memory_encoder = AutoModel.from_pretrained("sentence-transformers/all-mpnet-base-v2")
    ...
    async def store_multi_modal_episode(self, episode: Dict[str, Any], fusion_output: Dict[str, Any]):
        ...
        memory_entry = {
            ...
            "importance": self._calculate_multi_modal_importance(episode, fusion_output),
            "modality_snapshot": self._extract_modality_snapshot(fusion_output),
            "emotional_context": fusion_output.get("insights", {}).get("emotional_state", {}),
    ...
    def _calculate_multi_modal_importance(self, episode: Dict[str, Any], 
                                        fusion_output: Dict[str, Any]) -> float:
    ...
    def _extract_modality_snapshot(self, fusion_output: Dict[str, Any]) -> Dict[str, Any]:
        """Extract key features from each modality for memory"""
        return {
            "dominant_modality": fusion_output.get("insights", {}).get("attention_focus", {}).get("dominant_modality"),
            "modality_contributions": fusion_output.get("modality_contributions", {}),
            "representation_summary": fusion_output.get("representation").mean().item() if "representation" in fusion_output else 0
        }
    ...
    async def _update_multi_modal_semantic_memory(self, memory_entry: Dict[str, Any]):
        ...
        if "causal_factors" in memory_entry:
            for modality, factors in memory_entry["causal_factors"].get("causal_graph", {}).items():
...
class MultiModalContextAnalyzer:
    ...
    def __init__(self):
        ...
        # ML models
        try:
            self.activity_classifier = pipeline("zero-shot-classification", 
    ...
    async def analyze_comprehensive_context(self, multi_modal_inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if multi_modal_inputs:
            try:
                ...
                fusion_output = await self.unified_perception.perceive(multi_modal_inputs)
        ...
        user_state = await self._build_enhanced_user_state(context_signals, fusion_output)
        ...
        return {
            ...
            "timestamp": datetime.now(),
            "multi_modal_insights": fusion_output.get("insights", {}) if fusion_output else {}
        }
    ...
    async def _build_enhanced_user_state(self, signals: List[EnhancedContextSignal], 
                                        fusion_output: Optional[Dict[str, Any]]) -> 'EnhancedUserState':
        ...
        return EnhancedUserState(
            ...
            cognitive_load=cognitive_load,
            multi_modal_confidence=fusion_output.get("confidence", 0.0) if fusion_output else 0.0
        )
    ...
    async def _process_temporal_context(self) -> List[EnhancedContextSignal]:
        ...
        hour = now.hour
        if 6 <= hour < 9:
            signals.append(EnhancedContextSignal(
...
class EnhancedProactiveIntelligenceEngine:
    ...
    async def identify_proactive_opportunities(self, 
                                             context: Dict[str, Any],
                                             multi_modal_inputs: Optional[Dict[str, Any]] = None) -> List[ProactiveActionV2]:
        ...
        if multi_modal_inputs:
            ...
            if fusion_output:
                ...
                causal_opportunities = await self._identify_causal_opportunities(fusion_output)
                opportunities.extend(causal_opportunities)
                
                # Use uncertainty to identify clarification needs
                uncertainty_opportunities = await self._identify_uncertainty_opportunities(fusion_output)
                opportunities.extend(uncertainty_opportunities)
        
        # Traditional opportunity identification
        pattern_opportunities = await self._identify_pattern_based_opportunities(context)
        opportunities.extend(pattern_opportunities)
        
        predictive_opportunities = await self._identify_predictive_opportunities(context)
        opportunities.extend(predictive_opportunities)
        
        # Rank with multi-modal factors
        ranked_opportunities = await self._rank_opportunities_multimodal(opportunities, context)
        
        return ranked_opportunities
    
    async def _identify_causal_opportunities(self, fusion_output: Dict[str, Any]) -> List[ProactiveActionV2]:
    ...
    async def _identify_uncertainty_opportunities(self, fusion_output: Dict[str, Any]) -> List[ProactiveActionV2]:
    ...
    async def _rank_opportunities_multimodal(self, opportunities: List[ProactiveActionV2], 
                                           context: Dict[str, Any]) -> List[ProactiveActionV2]:
        ...
        for opp in opportunities:
            ...
            if fusion_output:
                ...
                modality_prefs = user_state.preferences.get("multi_modal_preferences", {})
                for modality in opp.multi_modal_requirements:
                    if modality == "voice" and modality_prefs.get("voice_feedback", True):
                        score *= 1.1
                    elif modality == "visual" and modality_prefs.get("visual_notifications", True):
                        score *= 1.1
    ...
    async def _identify_pattern_based_opportunities(self, context: Dict[str, Any]) -> List[ProactiveActionV2]:
    ...
    async def _identify_predictive_opportunities(self, context: Dict[str, Any]) -> List[ProactiveActionV2]:
    ...
    async def _optimize_multi_modal_routine(self, context: Dict[str, Any]) -> Dict[str, Any]:
...
class EnhancedPatternRecognizer:
    ...
    async def detect_patterns(self, recent_actions: deque, 
                            fusion_output: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """Detect patterns including multi-modal patterns"""
        patterns = []
...
class MultiModalActionPredictor:
    ...
    async def predict_next_actions(self, context: Dict[str, Any], 
                                  fusion_output: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        if fusion_output:
            emotional_state = fusion_output.get("insights", {}).get("emotional_state", {})
            ...
            if emotional_state.get("arousal", 0.5) > 0.7:
                predictions.append({
                    ...
                    "preparation": ["dim_lights", "play_calming_music", "suggest_break"],
...
class CausalActionReasoner:
    ...
    async def analyze_action_causality(self, action: ProactiveActionV2, 
                                     fusion_output: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for modality in action.multi_modal_requirements:
            if modality in causal_factors.get("intervention_effects", {}):
                predicted_effects[modality] = causal_factors["intervention_effects"][modality]
        
        return {
            "predicted_effects": predicted_effects,
            "confidence": len(predicted_effects) / len(action.multi_modal_requirements) if action.multi_modal_requirements else 0
        }
...
class EliteProactiveAssistantV2:
    ...
    async def start_proactive_assistance(self):
        ...
        logger.info("🚀 Starting JARVIS Elite Proactive Assistant v2.0 with Multi-Modal Fusion...")
    ...
    async def process_multi_modal_input(self, inputs: Dict[str, Any]):
        ...
        # Add to buffer
        self.multi_modal_buffer.append({
            "inputs": inputs,
            "timestamp": datetime.now()
        })
                ...
                if self.multi_modal_buffer:
                    ...
                    if context.get("fusion_output"):
                        await self.memory_system.store_multi_modal_episode(
                            {"inputs": recent_input["inputs"], "timestamp": recent_input["timestamp"]},
                            context["fusion_output"]
                        )
                    
                    # Identify multi-modal opportunities
                    opportunities = await self.intelligence_engine.identify_proactive_opportunities(
                        context,
                        recent_input["inputs"]
                    )
    ...
    async def _should_execute_multimodal(self, action: ProactiveActionV2, context: Dict[str, Any]) -> bool:
        ...
        if context.get("fusion_output"):
            uncertainty = context["fusion_output"].get("uncertainty", {}).get("total", 0.5)
            ...
            available_modalities = set(context["fusion_output"].get("modality_contributions", {}).keys())
    ...
    async def _continuous_context_monitoring(self):
        ...
        while self.running:
            try:
                ...
                opportunities = await self.intelligence_engine.identify_proactive_opportunities(context)
                
                # Filter and execute
                filtered_opportunities = await self._filter_opportunities(opportunities, context)
    ...
    async def _predictive_assistance_loop(self):
        ...
        while self.running:
            try:
                ...
                recent_mm_input = self.multi_modal_buffer[-1] if self.multi_modal_buffer else None
                if recent_mm_input:
                    mm_context = await self.context_analyzer.analyze_comprehensive_context(
                        recent_mm_input["inputs"]
                    )
                    predictions = await self.intelligence_engine.action_predictor.predict_next_actions(
                        mm_context,
                        mm_context.get("fusion_output")
                    )
                else:
                    predictions = await self.intelligence_engine.action_predictor.predict_next_actions(context)
                ...
                for prediction in predictions:
                    if prediction["probability"] > 0.8:
                        ...
                        if await self._should_execute(preparation_action, context):
                            await self._execute_proactive_action(preparation_action, context)
        ...
        while self.running:
            try:
                ...
                if mm_episodes:
                    ...
                    successful_mm = sum(1 for ep in mm_episodes 
                                      if ep.get("episode", {}).get("result", {}).get("success"))
                    self.metrics["multi_modal_accuracy"] = successful_mm / len(mm_episodes)
                    ...
                    for episode in mm_episodes[-10:]:
                        if "fusion_output" in episode:
                            ...
                            feedback = {"positive": episode["episode"].get("result", {}).get("success", False)}
                            
                            # Convert inputs to tensor format
                            tensor_inputs = {}
                            for modality, data in inputs.items():
                                if modality in self.unified_perception.processors:
                                    tensor_inputs[modality] = torch.randn(768)  # Placeholder
                ...
                logger.info(f"Learning update completed. MM Accuracy: {self.metrics['multi_modal_accuracy']:.2%}")
    ...
    async def _filter_opportunities(self, opportunities: List[ProactiveActionV2], 
                                  context: Dict[str, Any]) -> List[ProactiveActionV2]:
        ...
        for opportunity in opportunities:
            ...
            if hasattr(user_state, 'multi_modal_confidence'):
                if user_state.multi_modal_confidence < 0.5 and opportunity.action_type != "clarification":
        ...
        filtered.sort(key=lambda x: x.estimated_value / (x.estimated_disruption + 0.1), reverse=True)
        
        return filtered[:5]
    
    async def _check_user_preferences(self, action: ProactiveActionV2, 
                                    user_state: EnhancedUserState) -> bool:
        ...
        if action.multi_modal_requirements:
            if "voice" in action.multi_modal_requirements and not mm_prefs.get("voice_feedback", True):
                return False
            if "biometric" in action.multi_modal_requirements and not mm_prefs.get("biometric_monitoring", True):
                return False
    ...
    async def _should_execute(self, action: ProactiveActionV2, context: Dict[str, Any]) -> bool:
        ...
        if hasattr(user_state, 'cognitive_load') and user_state.cognitive_load > 0.8:
            if action.action_type not in ["assistance", "clarification", "stress_reduction"]:
                return False  # Only helpful actions when overloaded
        
        return True
    
    async def _execute_proactive_action(self, action: ProactiveActionV2, context: Dict[str, Any]):
        ...
        try:
            ...
            if context.get("fusion_output") and action.multi_modal_requirements:
                await self.memory_system.store_multi_modal_episode(episode, context["fusion_output"])
    ...
    async def _apply_optimization(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if opportunity["action"] == "organize_downloads":
            ...
            logger.info(f"Calibrating {opportunity.get('modalities', ['unknown'])[0]} modality")
