...
class SafeBuiltins:
    ...
    ALLOWED_BUILTINS = {
        ...
        'enumerate', 'filter', 'format', 'hex', 'isinstance', 'issubclass',
        'iter', 'len', 'map', 'max', 'min', 'next', 'oct', 'ord', 'pow',
        'range', 'repr', 'reversed', 'round', 'sorted', 'sum', 'zip',
...
class CodeValidator:
    ...
    FORBIDDEN_IMPORTS = {
        ...
        'shutil', 'pathlib', '__builtins__', 'eval', 'exec',
    ...
    FORBIDDEN_ATTRIBUTES = {
        ...
        '__module__', '__dict__', '__getattribute__', '__setattr__',
        '__delattr__', 'mro'
    }
    
    def __init__(self):
        self.allowed_imports = set(config_manager.get("security.allowed_imports", [
    ...
    def check_limits(self) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if cpu_percent > self.max_cpu_percent:
                return False, f"CPU limit exceeded: {cpu_percent:.1f}% > {self.max_cpu_percent}%"
...
class SecuritySandbox:
    ...
    async def _execute_docker(self, code: str, test_inputs: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _execute_subprocess(self, code: str, test_inputs: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _prepare_execution_script(self, code: str, test_inputs: Optional[Dict[str, Any]]) -> str:
        ...
        imports = "\n".join([
            f"import {module}" 
            for module in self.validator.allowed_imports
        ])
...
"""
    ...
    async def test_code(self, code: str, test_cases: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
                ...
                if 'expected' in test_case and 'variables' in result:
                    for var, expected in test_case['expected'].items():
                        ...
                        if not result['test_passed']:
                            result['test_error'] = f"Expected {var}={expected}, got {actual}"
                            break
