...
class PerformanceMetric(BaseModel):
    ...
    def update(self, exec_time: float, mem_usage: float, success: bool = True):
        ...
        self.memory_usage = (self.memory_usage * self.call_count + mem_usage) / (self.call_count + 1)
        self.call_count += 1
        if not success:
            self.error_count += 1
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    self.graph.add_node(
                        func_name,
                        type=ComponentType.COROUTINE if is_async else ComponentType.FUNCTION,
                        docstring=ast.get_docstring(node),
...
class RuntimeIntrospector:
    ...
    async def profile_component(self, component: Any, *args, **kwargs) -> Tuple[Any, PerformanceMetric]:
...
class DecisionAnalyzer:
    ...
    def _calculate_success_rate(self, decisions: List[Decision]) -> float:
        ...
        successful = sum(1 for d in decisions if d.outcome and 'success' in d.outcome.lower())
...
class MetaCognitiveIntrospector:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.root_path = Path(self.config.get('root_path', '/Users/andreprofitt/CloudAI/JARVIS-ECOSYSTEM'))
    ...
    async def build_self_model(self) -> Dict[str, Any]:
        ...
        self.self_model = {
            ...
            'health_score': self._calculate_health_score(runtime_profile, decision_patterns)
        }
    ...
    def _calculate_structural_metrics(self, graph: nx.DiGraph) -> Dict[str, Any]:
        ...
        return {
            'total_modules': len([n for n, d in graph.nodes(data=True) 
                                if d.get('type') == ComponentType.MODULE]),
            'total_classes': len([n for n, d in graph.nodes(data=True) 
                                if d.get('type') == ComponentType.CLASS]),
            'total_functions': len([n for n, d in graph.nodes(data=True) 
                                 if d.get('type') in [ComponentType.FUNCTION, ComponentType.COROUTINE]]),
            'avg_module_size': np.mean([d.get('loc', 0) for n, d in graph.nodes(data=True) 
                                       if d.get('type') == ComponentType.MODULE]),
            'complexity': {
    ...
    def _calculate_cohesion(self, graph: nx.DiGraph) -> float:
        ...
        module_nodes = [n for n, d in graph.nodes(data=True) 
                       if d.get('type') == ComponentType.MODULE]
    ...
    def _calculate_reliability(self) -> float:
        ...
        total_calls = sum(m.call_count for m in self.runtime_introspector.metrics.values())
        total_errors = sum(m.error_count for m in self.runtime_introspector.metrics.values())
        ...
        for node, data in structure.nodes(data=True):
            if data.get('type') in [ComponentType.FUNCTION, ComponentType.COROUTINE]:
                ...
                if docstring:
                    ...
                    keywords = ['process', 'analyze', 'generate', 'monitor', 'optimize', 
                              'learn', 'predict', 'classify', 'extract', 'transform']
    ...
    async def _identify_improvements(self, structure: nx.DiGraph, 
    ...
    def _calculate_health_score(self, runtime: Dict[str, Any], 
            ...
            try:
                ...
                if changes:
                    ...
                    for insight in new_insights:
                        if insight.actionable and insight.impact_score > self.insight_threshold:
                            await self.improvement_queue.put(insight)
                
                # Log health status
                health = self.self_model.get('health_score', 0) if self.self_model else 0
    ...
    async def capture_current_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'decisions': list(self.decision_analyzer.decision_history)[-100:],  # Recent 100
    ...
    async def detect_changes(self, current_state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for component, current_metric in current_metrics.items():
            if component in last_metrics:
                ...
                if abs(current_metric.execution_time - last.execution_time) / last.execution_time > 0.2:
        ...
        # Memory changes
        last_memory = self.self_model.get('runtime', {}).get('memory_snapshot', {}).get('total_memory', 0)
    ...
    async def analyze_causality(self, changes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for change_id, change in changes.items():
            if change['type'] == 'performance_change':
                ...
                recent_decisions = [d for d in self.decision_analyzer.decision_history 
                                  if d.component == component][-10:]
                
                causal_analysis[change_id] = {
                    'change': change,
                    'possible_causes': self._infer_causes(change, recent_decisions),
                    'correlation_strength': self._calculate_correlation(change, recent_decisions)
                }
                ...
                causal_analysis[change_id] = {
                    'change': change,
                    'possible_causes': ['increased_load', 'memory_leak', 'cache_growth'],
                    'correlation_strength': 0.7  # Simplified
                }
                
        return causal_analysis
    
    def _infer_causes(self, change: Dict[str, Any], decisions: List[Decision]) -> List[str]:
        ...
        if change.get('new_time', 0) > change.get('old_time', 0):
            ...
            optimization_actions = [d for d in decisions if 'optimize' in d.action.lower()]
            if optimization_actions:
                causes.append('successful_optimization')
                
        return causes
    
    def _calculate_correlation(self, change: Dict[str, Any], decisions: List[Decision]) -> float:
        ...
        for analysis_id, analysis in causal_analysis.items():
            ...
            if 'performance_degradation' in causes and correlation > 0.6:
                ...
                insights.append(Insight(
                    ...
                    impact_score=correlation,
                    actionable=False,
                    recommendations=["Document optimization approach for future reference"],
                    evidence={'change': change}
                ))
                
            elif change.get('type') == 'memory_change' and change.get('new_usage', 0) > change.get('old_usage', 0) * 1.5:
                insights.append(Insight(
                    category='memory',
    ...
    async def get_self_awareness_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'structural_complexity': self.self_model.get('structure', {}).get('metrics', {}),
            'performance_profile': {
                'bottlenecks': self.self_model.get('runtime', {}).get('bottlenecks', [])[:5],
            ...
            'capabilities_summary': {
                ...
                for capability, components in self.self_model.get('capabilities', {}).items()
            }
        }
...
async def main():
    ...
    for i in range(5):
        introspector.decision_analyzer.record_decision(Decision(
            timestamp=datetime.now(),
