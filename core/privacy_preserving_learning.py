...
class PrivacyAccountant:
    ...
    def _compute_subsampled_rdp(
        self, alpha: float, noise_scale: float, sampling_rate: float
    ) -> float:
        """Compute RDP for subsampled Gaussian mechanism"""
    ...
    def _log_binomial(self, n: float, p: float) -> float:
        ...
        if n > 50:
            ...
            return np.log(
                sum(
                    p**k * (1 - p) ** (n - k) * np.exp(self._log_comb(n, k))
    ...
    def _log_comb(self, n: float, k: int) -> float:
        ...
        try:
            ...
            return (
                math.log(math.factorial(int(n)))
                - math.log(math.factorial(k))
                - math.log(math.factorial(int(n) - k))
            )

    def _rdp_to_dp(
        self, alphas: np.ndarray, rdp_epsilons: np.ndarray, delta: float
    ) -> float:
    ...
    def add_privacy_expense(self, epsilon: float):
        ...
        logger.info(
            f"Privacy expense added: ε={epsilon:.4f}, "
    ...
    def _compute_total_privacy(self) -> float:
        ...
        k = len(epsilons)
        advanced_composition = np.sqrt(2 * k * np.log(1 / self.config.delta)) * np.max(
            epsilons
        ) + k * np.max(epsilons) * (np.exp(np.max(epsilons)) - 1)
...
class SecureAggregator:
    ...
    def generate_client_keys(self, client_id: str) -> bytes:
        ...
        try:
            ...
            logger.warning(
                "Cryptography library not available, using simple encryption"
            )
    ...
    async def secure_aggregate(
        self, encrypted_updates: Dict[str, bytes], weights: Dict[str, float]
    ) -> torch.Tensor:
        """
        ...
        for client_id, encrypted_update in encrypted_updates.items():
            if client_id in self.encryption_keys:
                try:
                    ...
                    # Fallback decryption
                    update = self._simple_decrypt(
                        encrypted_update, self.encryption_keys[client_id]
                    )
    ...
    def _simple_decrypt(self, encrypted_data: bytes, key: bytes) -> torch.Tensor:
        ...
        decrypted = bytes(
            a ^ b
            for a, b in zip(encrypted_data, key * (len(encrypted_data) // len(key) + 1))
        )
        return torch.tensor(json.loads(decrypted))

    def verify_aggregation(
        self, aggregated_model: torch.Tensor, client_updates: List[torch.Tensor]
    ) -> bool:
...
class DifferentialPrivacyMechanism:
    ...
    def clip_gradients(
        self, gradients: torch.Tensor, clip_threshold: float
...
class FederatedClient:
    ...
    def __init__(self, client_id: str, model: nn.Module, privacy_config: PrivacyConfig):
                ...
                for param in self.model.parameters():
                    if param.grad is not None:
                        param.grad.data, _ = self.dp_mechanism.clip_gradients(
                            param.grad.data, self.privacy_config.gradient_clip_threshold
                        )
    ...
    async def secure_delete_local_data(self):
        ...
        if self.local_data is not None:
            ...
            data_size = (
                self.local_data.nbytes if hasattr(self.local_data, "nbytes") else 0
            )
    ...
    def encrypt_update(self, update: Dict[str, Any], key: bytes) -> bytes:
        ...
        try:
            ...
            serialized = json.dumps(
                {k: v.tolist() if torch.is_tensor(v) else v for k, v in update.items()}
            )
            ...
            encrypted = bytes(
                a ^ b
                for a, b in zip(
                    serialized.encode(), key * (len(serialized) // len(key) + 1)
                )
            )
            return encrypted
...
class PrivacyPreservingLearningSystem:
    ...
    def __init__(
        ...
        # Metrics tracking
        self.metrics = {"rounds": [], "privacy_budget": [], "accuracy": [], "loss": []}

    async def initialize_clients(
        self, num_clients: int, data_distribution
    ...
    async def select_clients(
        self, clients: List[FederatedClient], num_select: int
    ) -> Tuple[List[FederatedClient], float]:
        """
        Randomly select clients for training round
        Implements privacy amplification through subsampling
        """
        selected_indices = np.random.choice(
            len(clients), size=min(num_select, len(clients)), replace=False
        )
    ...
    async def federated_round(
        self, clients: List[FederatedClient], round_num: int
    ) -> Dict[str, Any]:
        """
        ...
        selected_clients, sampling_rate = await self.select_clients(
            clients, self.federated_config.clients_per_round
        )
        ...
        for update in updates:
            ...
            if self.privacy_config.secure_aggregation:
                ...
                encrypted = client.encrypt_update(
                    update["model_update"], client.encryption_key
                )
    ...
    def _weighted_average(
        self, updates: Dict[str, Dict[str, torch.Tensor]], weights: Dict[str, float]
    ...
    async def train(
        self, clients: List[FederatedClient], test_loader=None
    ) -> Dict[str, List]:
        """
        ...
        logger.info(
            f"Privacy budget: ε={self.privacy_config.epsilon}, "
        ...
        for round_num in range(self.federated_config.num_rounds):
            ...
            # Log progress
            logger.info(
                f"Round {round_num + 1}/{self.federated_config.num_rounds}: "
        ...
        logger.info(
            f"Final privacy spent: ε={self.privacy_accountant.total_epsilon:.4f}"
        )
    ...
    def get_privacy_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "training_stats": {
                ...
                "final_accuracy": (
                    self.metrics["accuracy"][-1] if self.metrics["accuracy"] else None
                ),
                "privacy_efficiency": (
                    self.metrics["accuracy"][-1] / self.privacy_accountant.total_epsilon
                    if self.metrics["accuracy"]
                    and self.privacy_accountant.total_epsilon > 0
                    else None
                ),
            },
        }
...
async def demo_privacy_preserving_learning():
    ...
    system = PrivacyPreservingLearningSystem(
        model_fn=model_fn, privacy_config=privacy_config, federated_config=fed_config
    )
    ...
    print(
        f"   - Privacy efficiency: {report['training_stats']['privacy_efficiency']:.4f}"
    )
...
def calculate_optimal_noise_scale(
    epsilon: float, delta: float, sensitivity: float, num_rounds: int
) -> float:
    """
