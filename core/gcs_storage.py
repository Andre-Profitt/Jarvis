...
class GCSStorageManager:
    ...
    def _initialize_client(self):
        ...
        if not GCS_AVAILABLE:
            logger.warning(
                "Google Cloud Storage library not installed. Using local fallback."
            )
            return
    ...
    @retry.Retry()
    def download(
        self, source: str, destination: Optional[Union[str, BinaryIO]] = None
    ) -> Union[bytes, str]:
        """
    ...
    def save_model(
        ...
        # Upload model
        uri = self.upload(
            model_bytes, path, metadata=meta, content_type="application/octet-stream"
        )
    ...
    async def upload_async(
        self, source: Union[str, bytes], destination: str, **kwargs
    ) -> str:
        """Async wrapper for upload"""
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self._executor, lambda: self.upload(source, destination, **kwargs)
        )
    ...
    async def download_async(
        ...
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self._executor, lambda: self.download(source, destination)
        )
...
def download_from_gcs(
    source: str, destination: Optional[str] = None
) -> Union[bytes, str]:
