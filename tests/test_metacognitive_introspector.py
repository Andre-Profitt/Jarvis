...
''')
    ...
    async def test_codebase_analysis(self, temp_project):
        ...
        func_nodes = [n for n, d in graph.nodes(data=True) 
                     if d.get('type') in [ComponentType.FUNCTION, ComponentType.COROUTINE]]
        assert len(func_nodes) >= 3
        
        # Check imports
        import_edges = [(u, v) for u, v, d in graph.edges(data=True) 
...
class TestMetaCognitiveIntrospector:
    ...
    def test_identify_bottlenecks(self, temp_introspector):
        ...
        # Add test metrics
        temp_introspector.runtime_introspector.metrics['slow_component'] = PerformanceMetric(
            execution_time=2.0,
            memory_usage=50,
            call_count=10
        )
        
        temp_introspector.runtime_introspector.metrics['memory_hog'] = PerformanceMetric(
            execution_time=0.5,
            memory_usage=200,
            call_count=5
        )
...
class TestMetaCognitiveJARVIS:
    ...
    def mock_subsystems(self):
        ...
        neural = Mock()
        neural.get_system_stats = AsyncMock(return_value={
            'cpu_usage': 50,
            'memory_usage': 60,
        ...
        self_healing = Mock()
        self_healing.get_system_status = AsyncMock(return_value={
            'success_rate': 0.9,
            'anomaly_rate': 0.1
        })
    ...
    async def test_improvement_plan_generation(self, mock_subsystems):
        ...
        mc_jarvis.introspector.insights.append(Insight(
