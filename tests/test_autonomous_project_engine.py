...
class MockAnalyzerAgent(AnalyzerAgent):
    async def _analyze_requirements(self, task, context):
        return {
            "functional_requirements": ["mock_req1"],
...
async def test_agent_registration(mock_engine):
    ...
    assert isinstance(
        mock_engine.orchestrator.agent_registry["mock_analyzer_01"], MockAnalyzerAgent
    )
    assert isinstance(
        mock_engine.orchestrator.agent_registry["mock_qa_01"], MockQAAgent
    )
