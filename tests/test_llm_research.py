...
from core.llm_research_jarvis import ResearchTask, LLMResearchJARVIS
    ...
    async def test_claude_analyze(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout='{"analysis": "Test analysis", "confidence": 0.9}'
            )
...
class TestGeminiCLI:
    ...
    async def test_gemini_analyze(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout="Analysis result from Gemini"
            )
    ...
    async def test_gemini_validate(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0, stdout='{"is_valid": true, "confidence": 0.95}'
            )

            response = await gemini.validate("Test claim", ["evidence 1", "evidence 2"])
...
class TestResearchTask:
    ...
    def test_task_creation(self):
        ...
        task = ResearchTask(
            id="test123", topic="AI Safety", type="deep", depth="comprehensive"
        )
    ...
    def test_task_with_questions(self):
        ...
        questions = ["Q1", "Q2", "Q3"]
        task = ResearchTask(
            id="test456", topic="Quantum ML", custom_questions=questions
        )
...
class TestLLMResearchJARVIS:
    ...
    async def test_initialization(self):
        ...
        with (
            patch("core.llm_research_jarvis.ClaudeCLI"),
    ...
    async def test_research_types(self):
        ...
        # Test deep research
        task = ResearchTask(
            id="deep1", topic="Deep Learning", type="deep", depth="comprehensive"
        )
        ...
        hypothesis_task = ResearchTask(
            id="hyp1", topic="AGI Development", type="hypothesis"
        )
...
class TestQuickResearch:
    ...
    async def test_quick_research(self):
        ...
        with patch(
            "core.llm_research_jarvis.llm_research_jarvis.research"
        ) as mock_research:
...
async def test_end_to_end_research():
    ...
    with (
        ...
        # Mock CLI responses
        mock_run.return_value = MagicMock(
            returncode=0, stdout='{"result": "Mocked LLM response"}'
        )
        ...
        # Conduct research
        result = await jarvis.research("Test research topic", depth="quick")
